...
namespace samptool
{
    sealed class Sample
    {
        ...
        internal void InternalReadData(MemoryMappedViewAccessor sdirAccess, MemoryMappedViewAccessor sampAccess, ref long position)
        {
            // read header
            sdirAccess.ReadUInt16BE(ref position, out m_header.id);
            sdirAccess.ReadUInt16BE(ref position, out m_header.field02);
            sdirAccess.ReadUInt32BE(ref position, out m_header.dataOffset);
            sdirAccess.ReadUInt32BE(ref position, out m_header.field08);
            sdirAccess.ReadUInt16BE(ref position, out m_header.field0C);
            sdirAccess.ReadUInt16BE(ref position, out m_header.sampleRate);
            sdirAccess.ReadUInt32BE(ref position, out m_header.numSamples);
            sdirAccess.ReadUInt32BE(ref position, out m_header.loopStart);
            sdirAccess.ReadUInt32BE(ref position, out m_header.loopLength);
            sdirAccess.ReadUInt32BE(ref position, out m_header.metadataOffset);

            // read metadata
            // make a copy of the offset so we won't modify the original value
            long metadataOffset = m_header.metadataOffset;

            sdirAccess.ReadUInt16BE(ref metadataOffset, out m_metadata.field00);
            sdirAccess.ReadByte(ref metadataOffset, out m_metadata.predictorScale);
            sdirAccess.ReadByte(ref metadataOffset, out m_metadata.loopPredictorScale);
            sdirAccess.ReadUInt16BE(ref metadataOffset, out m_metadata.loopSampleHistory2);
            sdirAccess.ReadUInt16BE(ref metadataOffset, out m_metadata.loopSampleHistory1);

            m_metadata.coefficients = new ushort[16];
            for (int i = 0; i < m_metadata.coefficients.Length; i++)
                sdirAccess.ReadUInt16BE(ref metadataOffset, out m_metadata.coefficients[i]);
        ...
        internal void InternalWriteMetadata(MemoryMappedViewAccessor access, ref long position)
        {
        ...
        // write info header
        internal void InternalWriteHeader(MemoryMappedViewAccessor access, ref long position)
        {
        ...
        internal struct SampleDecodingMetadata
        {
